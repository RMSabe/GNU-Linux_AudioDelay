...
#define BYTEBUF_SIZE_BYTES (3U*BUFFER_SIZE_BYTES/4U)
...
void buffer_malloc(void)
{
	...
	buffer_output_0 = (int*) malloc(BUFFER_SIZE_BYTES);
	buffer_output_1 = (int*) malloc(BUFFER_SIZE_BYTES);
	buffer_output_2 = (int*) malloc(BUFFER_SIZE_BYTES);
	buffer_output_3 = (int*) malloc(BUFFER_SIZE_BYTES);
...
void run_dsp(void)
{
	...
	{
		load_delay();
		load_out[2*n_frame] = ((curr_in[2*n_frame]) + (buffer_dsp[2*n_frame]))/2;
		load_out[2*n_frame + 1] = ((curr_in[2*n_frame + 1]) + (buffer_dsp[2*n_frame + 1]))/2;
...
void load_delay(void)
{
	...
	if((l_sample < SAMPLE_MAX_VALUE) && (l_sample > SAMPLE_MIN_VALUE)) buffer_dsp[2*n_frame] = l_sample;
	else if(l_sample >= SAMPLE_MAX_VALUE) buffer_dsp[2*n_frame] = SAMPLE_MAX_VALUE;
	else if(l_sample <= SAMPLE_MIN_VALUE) buffer_dsp[2*n_frame] = SAMPLE_MIN_VALUE;

	if((r_sample < SAMPLE_MAX_VALUE) && (r_sample > SAMPLE_MIN_VALUE)) buffer_dsp[2*n_frame + 1] = r_sample;
	else if(r_sample >= SAMPLE_MAX_VALUE) buffer_dsp[2*n_frame + 1] = SAMPLE_MAX_VALUE;

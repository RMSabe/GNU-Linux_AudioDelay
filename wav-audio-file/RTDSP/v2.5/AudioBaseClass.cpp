...
bool AudioBaseClass::setParameters(const audio_rtdsp_params_t *p_params)
{
	...
	if(p_params->filein_dir == NULL) return false;
...
void AudioBaseClass::playback_init(void)
{
	...
	this->userthread = std::thread(&AudioBaseClass::userthread_proc, this);
	return;
}
...
void AudioBaseClass::playback_loop(void)
{
	while(!this->stop)
	{
		this->playthread = std::thread(&AudioBaseClass::playthread_proc, this);
		this->loadthread = std::thread(&AudioBaseClass::loadthread_proc, this);
		cppthread_wait(&this->loadthread);
		cppthread_wait(&this->playthread);
...
void AudioBaseClass::buffer_segment_remap(void)
{
	...
	if(this->bufferin_nsegment_curr >= this->BUFFERIN_N_SEGMENTS) this->bufferin_nsegment_curr = 0u;

	this->bufferout_nsegment_load++;
	if(this->bufferout_nsegment_load >= this->BUFFEROUT_N_SEGMENTS) this->bufferout_nsegment_load = 0u;

	this->bufferout_nsegment_play++;
	if(this->bufferout_nsegment_play >= this->BUFFEROUT_N_SEGMENTS) this->bufferout_nsegment_play = 0u;
...
void AudioBaseClass::loadthread_proc(void)
{
	this->buffer_load();
...
void AudioBaseClass::playthread_proc(void)
{
	this->buffer_play();
	return;
}

void AudioBaseClass::userthread_proc(void)
{
	int n_ret = 0;

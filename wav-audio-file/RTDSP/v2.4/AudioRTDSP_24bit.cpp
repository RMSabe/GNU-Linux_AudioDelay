...
AudioRTDSP_24bit::AudioRTDSP_24bit(const audio_rtdsp_params_t *p_params) : AudioBaseClass(p_params)
{
}

AudioRTDSP_24bit::~AudioRTDSP_24bit(void)
{
...
bool AudioRTDSP_24bit::audio_hw_init(void)
{
...
bool AudioRTDSP_24bit::buffer_alloc(void)
{
	size_t n_seg = 0u;
	...
	this->p_bytebuf = (std::uint8_t*) std::malloc(this->BYTEBUF_SIZE);
...
void AudioRTDSP_24bit::buffer_free(void)
{
	if(this->p_bufferin != nullptr)
	{
...
void AudioRTDSP_24bit::buffer_load(void)
{
...
void AudioRTDSP_24bit::dsp_proc(void)
{
	std::int32_t *currin_seg = nullptr;
	std::int32_t *previn_seg = nullptr;
	std::int32_t *loadout_seg = nullptr;
	...
	currin_seg = (std::int32_t*) this->pp_bufferin_segments[this->bufferin_nsegment_curr];
	loadout_seg = (std::int32_t*) this->pp_bufferout_segments[this->bufferout_nsegment_load];
	...
	for(currin_seg_nframe = 0u; currin_seg_nframe < this->AUDIOBUFFER_SIZE_FRAMES; currin_seg_nframe++)
	{
		...
		while(n_cycle <= n_cycles)
		{
			...
			previn_seg = (std::int32_t*) this->pp_bufferin_segments[previn_nseg];
